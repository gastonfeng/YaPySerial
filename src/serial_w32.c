...
static serial_t *serial_open(const char *device, const int timeout )
{
    serial_t *h = calloc(sizeof(serial_t), 1);
    char *devName;

    /* timeout in ms */
    COMMTIMEOUTS timeouts = {MAXDWORD, MAXDWORD,  timeout * 100, 0, 0};
...
static int serial_setup(serial_t *h,
...
int yapy_serial_open( void **port, const char *device, const unsigned int baud, const char * serial_mode, const int timeout )
{
    serial_t *h;
    int dev_len, dev_pos;
    ...
    else if( !strncmp(device, "\\\\.\\COM", strlen("\\\\.\\COM") ) )
    {
        dev_pos = strlen("\\\\.\\COM");
    }
    else
    ...
    while( dev_pos < dev_len )
    {
        if( !isdigit( device[dev_pos++] ) )
        {
            return PORT_ERR_NODEV;
        }
    }
    ...
    /* 2. check options */
    if (serial_get_baud(baud) == SERIAL_BAUD_INVALID)
        return PORT_ERR_UNKNOWN;
    if (serial_get_bits(serial_mode) == SERIAL_BITS_INVALID)
        return PORT_ERR_UNKNOWN;
    if (serial_get_parity(serial_mode) == SERIAL_PARITY_INVALID)
        return PORT_ERR_UNKNOWN;
    if (serial_get_stopbit(serial_mode) == SERIAL_STOPBIT_INVALID)
        return PORT_ERR_UNKNOWN;

    /* 3. open it */
    h = serial_open(device, timeout);
    if (h == NULL)
        return PORT_ERR_UNKNOWN;

    /* 4. set options */
    if (serial_setup(h,
                     serial_get_baud(baud),
...
int yapy_serial_close(void **port)
{
    serial_t *h;
...
int yapy_serial_read(void **port, void *buf,
                     size_t nbyte)
{
    serial_t *h;
    DWORD r;
...
int yapy_serial_write(void **port, void *buf,
                      size_t nbyte)
{
    serial_t *h;
    DWORD r;
...
int yapy_serial_gpio(void **port,
                     int n, int level)
{
    serial_t *h;
    int bit;
...
const char *yapy_serial_get_cfg_str(void **port)
{
    serial_t *h;
